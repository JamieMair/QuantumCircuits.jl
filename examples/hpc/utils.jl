...
function create_nn_from_architecture(config)
    ...
    hamiltonian_noise = haskey(config, :hamiltonian_noise) ? config[:hamiltonian_noise] : 0
    ...
    if !iszero(hamiltonian_noise)
        hamiltonian_layer = NoisyHamiltonianLayer(hamiltonian_noise, hamiltonian_layer)
    end
