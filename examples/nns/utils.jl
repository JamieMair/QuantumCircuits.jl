...
function plot_gradient_norms(dfs...; plot_log=false)
    nbits_set = sort(unique(vcat((unique(df[!, :c_nbits]) for df in dfs)...)))
    ...
    for (j, df) in enumerate(dfs)
        ...
        for (i, nbits) in enumerate(nbits_set)
            ...
            xlabel = i < length(nbits_set) ? "" : L"t"
            ylabel = j > 1 ? "" : L"\mathbb{E} \left [ \left {||} \overline{\nabla_\theta H} \right {||} \right ]"
            ax = Axis(f[i, j]; xlabel, ylabel, title=LaTeXString("\$n=$nbits, |\\theta|\\approx 10^{$nparams_log10}\$"), additional_args...)
            ...
            for nlayers in nlayers_local_set
                trajectories = map(subset(df,
                    :c_nbits => nb -> nb .== nbits,
                    :c_nlayers => nl -> nl .== nlayers)[!, :r_training_info]) do info
                        epochs = sort([k for k in keys(info[:gradient_info])])
                        return map(epochs) do i
                            info[:gradient_info][i].norm_grads # assume same epochs
                        end
                end
                ...
                mean_trajectory = reshape(mean(combined_trajs, dims=2), :)
                err_trajectory = reshape(std(combined_trajs, dims=2), :) / sqrt(length(trajectories))
                epochs = 0:(length(mean_trajectory)-1)
